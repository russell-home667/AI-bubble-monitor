...
def call_search(key,mode,focus):
 ...
 prompt=f'''Search the web for AI-bubble-relevant events published since {since} Singapore time.\nFocus: {focus}\nAllowed categories: {', '.join(CATS)}.\nPrioritize Reuters, Bloomberg, Financial Times, WSJ, CNBC, AP and primary official sources. Use TechCrunch/The Information/Semafor/Fortune only when useful.\nReturn only events with importance_score >= 50. Each source URL must be an exact page you found through web search.\nReturn JSON exactly as {{"stories":[{{"headline":"...","summary_zh":"...","category":"...","importance_score":0,"bubble_direction":"risk_up|risk_down|neutral","bubble_risk_score":0,"reason_zh":"...","companies":["..."],"published_at":"ISO-8601 with timezone if known","sources":[{{"name":"Reuters","url":"https://..."}}]}}]}}.'''
 body={'model':MODEL,'instructions':instr,'input':prompt,'tools':[{'type':'web_search'}],'tool_choice':{'type':'web_search'},'reasoning':{'effort':'high' if deep else 'low'},'max_output_tokens':16000 if deep else 10000,'text':{'format':{'type':'json_object'}}}
