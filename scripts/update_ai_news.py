...
UA='Mozilla/5.0 (compatible; AI-Bubble-Monitor/1.1; +https://github.com/russell-home667/AI-bubble-monitor)'
...
def trusted_source_obj(source):
 return domaininfo(source.get('domain')) or hostinfo(source.get('url',''))
...
def dtparse(s):
 ...
 try:
  d=parsedate_to_datetime(text);return d.astimezone(SGT)
 except:return None
...
def bing_page(query,interval,first):
 ...
 for item in root.findall('.//item'):
  ...
  out.append({'title':title,'url':url,'source':hi[1],'domain':hi[0],'published_at':when.isoformat(timespec='minutes'),'description':desc[:900],'discovery_source':'Bing News RSS'})
 return out

def google_page(query,deep):
 q=f'{query} when:{"7d" if deep else "2d"}'
 r=requests.get(GOOGLE,params={'q':q,'hl':'en-US','gl':'US','ceid':'US:en'},headers={'User-Agent':UA,'Accept':'application/rss+xml,application/xml,text/xml,*/*'},timeout=30)
 r.raise_for_status();root=ET.fromstring(r.content);out=[]
 for item in root.findall('.//item'):
  title=clean_html(item.findtext('title'));url=(item.findtext('link') or '').strip();when=dtparse(item.findtext('pubDate'));desc=clean_html(item.findtext('description'))
  src=item.find('source');src_url=(src.attrib.get('url','').strip() if src is not None else '');src_name=clean_html(src.text if src is not None else '')
  hi=hostinfo(src_url) or domaininfo(urlparse(src_url).hostname or '')
  if not title or not url or not when or not hi:continue
  out.append({'title':title,'url':url,'source':hi[1] or src_name,'domain':hi[0],'published_at':when.isoformat(timespec='minutes'),'description':desc[:900],'discovery_source':'Google News RSS'})
 return out
...
def gdelt_page(query,deep):
 ...
 for a in data.get('articles',[]) or []:
  title=clean_html(a.get('title'));url=str(a.get('url') or '').strip();when=dtparse(a.get('seendate'))
  hi=domaininfo(a.get('domain')) or hostinfo(url)
  if not title or not url or not when or not hi:continue
  out.append({'title':title,'url':url,'source':hi[1],'domain':hi[0],'published_at':when.isoformat(timespec='minutes'),'description':'','discovery_source':'GDELT DOC API'})
 return out
 ...
 for cat,q in QUERIES.items():
  ...
  for batch in batches:
   for x in batch:
    ...
    if d and d>=cut:
     x['query_category']=cat;raw.append(x);discovery_counts[x['discovery_source']]=discovery_counts.get(x['discovery_source'],0)+1
  time.sleep(.20)
 raw.sort(key=lambda x:x['published_at'],reverse=True);res=[];seen=set();per_domain={}
 cap=28 if deep else 14;limit=260 if deep else 150
 for x in raw:
  u=x['url'].split('#')[0]
  if u in seen or any(sim(x['title'],z['title'])>.90 for z in res[-100:]):continue
...
def analyze(key,mode,cands):
 ...
 for s in raw:
  ...
  for cid in ids[:8]:
   ...
   su.add(key);sources.append({'name':x['source'],'domain':x['domain'],'url':x['url'],'published_at':x['published_at'],'discovered_via':x.get('discovery_source')})
...
def merge(new,existing):
 ...
 for s in new+existing:
  ...
  if not any(trusted_source_obj(z) for z in s.get('sources',[])):continue
  pool.append(dict(s))
