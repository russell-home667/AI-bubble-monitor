(() => {
  ...
  // Source links point to the human-readable page that matches the actual data lineage.
  const S = {
    methodology: {label:'Methodology / lineage', url:'backtest.html', internal:true},

    // Yahoo market / intraday quote layer.
    ...
    // Official Treasury close layer.
    treasuryNominal: {
      label:'U.S. Treasury · Daily Treasury Par Yield Curve',
      url:'https://home.treasury.gov/resource-center/data-chart-center/interest-rates/TextView?type=daily_treasury_yield_curve'
    },
    treasuryReal: {
      label:'U.S. Treasury · Daily Treasury Par Real Yield Curve',
    ...
    // Credit remains on the existing FRED / ICE BofA series.
    ...
    // H5 quarterly financial statements.
  ...
  const BACKTEST = [S.yahooNDX,S.yahooSOX,S.yahooNVDA,S.yahooQQQ,S.yahooRSP,S.yahooVIX,S.fredBAA,S.treasuryReal,S.tsmcLanding];
  ...
  function syncRatesSourceText(){
    document.querySelectorAll('#marketCards .card').forEach(card=>{
      if(card.dataset.rateSourceSynced==='1') return;
      const name=norm(card.querySelector('.metric-name')?.textContent);
      ...
      if(name.includes('us 10y treasury')){
        ...
        if(!detail.textContent.startsWith('U.S. Treasury Real Yield Curve')){
          detail.textContent='U.S. Treasury Real Yield Curve · '+detail.textContent;
        }
        card.dataset.rateSourceSynced='1';
      }
    });
  ...
  function ruleFor(raw){
    ...
    if (t === 'vix' || t.includes('cboe vix')) return one(S.yahooVIX);
    if (t.includes('market prices') || t.includes('market price trend') || t.includes('market trend')) return many(MARKET);

    if (t.includes('30y treasury') || t.includes('30-year treasury') || t.includes('dgs30')) return many([S.yahooTYX,S.treasuryNominal]);
    if ((t.includes('10y treasury') || t.includes('10-year treasury') || t.includes('dgs10')) && !t.includes('real')) return many([S.yahooTNX,S.treasuryNominal]);
    if (t.includes('10y real yield') || t.includes('10-year real') || t.includes('dfii10')) return one(S.treasuryReal);
